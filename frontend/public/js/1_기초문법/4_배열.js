/*
	변수 : 데이터 1개를 저장할 수 있는 공간(메모리)
		변수 선언 : let 변수명 = 데이터
	상수 : 데이터 1개를 저장할 수 있는 공간(메모리)
		선언키워드 변수명 = 저장할데이터
		
	배열 : 데이터 여러개를 한 곳에 저장할 수 있는 공간(메모리)	
		- 순서대로 번호가 붙은 데이터들의 연속적인 형태
		- 순서대로 번호 = *인덱스*
		- 인덱스 순으로 데이터들을 연속적으로 저장
		1. 배열 : [] 대괄호 사용
		2. 식별번호 : 인덱스사용 [ 0번부터 시작 ~ ]
		3. 배열의 선언
			let 배열명 = [데이터1, 데이터2, 데이터3]
			const 배열명 = [데이버1, 데이터2, 데이터3]
			ex)
			let 청소부 = '하하'
			=> '청소부'라는 변수명에 1명/1개 데이터를 저장함[변수]
			let 무궁화반 = ['유재석', '강호동', '신동엽', '서장훈']
			=> '무궁화반'이라는 변수명에 4명/4개 데이터를 저장함[배열]
		...
		7. 배열내 데이터 추가
			배열명.push(새로운값)	:	배열내 마지막 위치에 저장
				- 마지막 인덱스 뒤에 데이터 추가
			배열명.splice(추가할위치인덱스, 0, 새로운값)	:	해당 위치인덱스에 새로운 값 저장
				- 특정 위치/인덱스 에 데이터 추가
		8. 배열내 데이터 삭제
		...
		9.	배열내 데이터를 이용한 데이터 삭제
			- 배열명.indexOf('찾을데이터') : 찾을데이터가 존재하면 찾은 인덱스번호 찾지못하면 -1
			ex) 배열명.splice(배열명.indexOf('바나나'), 1)	: '바나나'가 존재하면 삭제	
...
document.write('</ol>')
// 예제4 : 인덱스를 이용한 데이터 수정
document.write( 배열명 + '<br>')
배열명[0] = '수박'
document.write( 배열명 + '<br>')
// 예제5 : 배열에 요소/데이터 추가
배열명.push('체리')		// 배열내 마지막 요소/데이터 뒤에 새로운 데이터 추가
console.log(배열명)
document.write( 배열명 + '<br>')
배열명.splice(0 , 2, '두리안')	// 0번째 인덱스에서 2개 자르고 '두리안' 추가
console.log(배열명)
document.write( 배열명 + '<br>')
// 예제6 : 배열에 특정 요소/데이터 삭제
배열명.splice(0, 1)	// 0번 인덱스부터 1개 데이터 삭제
console.log(배열명)
document.write( 배열명 + '<br>')
배열명.splice(배열명.indexOf('딸기'), 1)	// '딸기'데이터를 찾아 삭제
console.log(배열명)
document.write( 배열명 + '<br>')
/*commit*/
